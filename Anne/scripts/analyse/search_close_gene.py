...
def search_gene(list_chr, path_analyse, type_file, non_coding, type_MTC):
    path_gene_file = "D:/Hanze_Groningen/STAGE/db/all_genes_new - kopie.tsv"
    df = pd.read_csv(path_gene_file, sep='\t')
    print(df)
    print(df.columns)
    # # df.iloc[(df['num']-input).abs().argsort()[:2]]
    f = open(f"{path_analyse}correction/{type_file}_{non_coding}_{type_MTC}_close_gene.tsv", "a")
    f.write(f"snp_ID\tchr\tpos_begin\tpos_end\thg19.kgXref.geneSymbol\tposition_region_to_gene\tdistance_gene\tcolumn_name_min\n")
    for pos in list_chr:
        print()
        print(pos)
        if len(pos.split('_')) == 3:
            chr, pos_begin, pos_end = pos.split('_')
            snp_ID = '-'
        if len(pos.split('_')) == 4:
            snp_ID, chr, pos_begin, pos_end = pos.split('_')
        ...
        if len(df_filter) < 1:
            ...
            if len(df_filter) < 1:
                ...
                f.write(f"{snp_ID}\t{chr}\t{pos_begin}\t{pos_end}\t{df_filter.iloc[index_min]['hg19.kgXref.geneSymbol']}\tout\t{value_min}\t{colnames_min[ind]}\n")
            else:
                print('overlap')
                print(list(df_filter['hg19.kgXref.geneSymbol']))
                f.write(f"{snp_ID}\t{chr}\t{pos_begin}\t{pos_end}\t{','.join(map(str, list(set(list(df_filter['hg19.kgXref.geneSymbol'])))))}\toverlap\t0\t-\n")
        else:
            print('in')
            print(list(df_filter['hg19.kgXref.geneSymbol']))
            f.write(f"{snp_ID}\t{chr}\t{pos_begin}\t{pos_end}\t{','.join(map(str, list(set(list(df_filter['hg19.kgXref.geneSymbol'])))))}\tin\t0\t-\n")
    f.close()
