...
make_plots <- function(chr, filenames, df, i, basename_file, counter, r0, r1, last_par, new_add, kp, num_for_r, custom.cytobands) {
  ...
  if (new_add == 'new') {
    ...
    kp <- plotKaryotype()
    # Select chromosome
    kp <- plotKaryotype(genome="hg19", plot.type=1, chromosomes=c(chr), cytobands = custom.cytobands) 
...
read_files <- function(filenames, chr, r0, r1, path_info_save, num_of_pictures, num_for_r, custom.cytobands) {
  ...
  for (i in 1:length(filenames)){  
    ...
    if ( (i%%(num_of_pictures+1)) == 0 || i == 1){
      ...
      # Call make_plots
      info <- make_plots(chr, filenames, df, i, basename_file, counter, r0, r1, '', 'new', '', num_for_r, custom.cytobands) #kp, df, counter
      ...
      # Call make_plots
      info <- make_plots(chr, filenames, df, i, basename_file, counter, r0, r1, '', 'add', kp, num_for_r, custom.cytobands) #kp, df, counter
      # The kp plot (karyoplot object)
      kp <- info[[1]]
      # The data frame that is continuously expanded with new data for the SUM plot
      df <- info[[2]]
...
main <- function() {
  ...
  for (chr in chrom){
    ...
    # Call read_files      
    read_files(filenames, chr, r0, r1, path_info_save, num_of_pictures, num_for_r, custom.cytobands)        
  }
}
