...
# Make new directory
mkdir -p /groups/umcg-wijmenga/tmp01/projects/lude_vici_2021/rawdata/non-coding-somatic-mutations-in-cancer/Anne/scripts/pipeline_variant_calling/jobs/

# Loop over samples
for i in "${!SAMPLES[@]}"
do  
    # Path to the jobs
    JOB_PATH=/groups/umcg-wijmenga/tmp01/projects/lude_vici_2021/rawdata/non-coding-somatic-mutations-in-cancer/Anne/scripts/pipeline_variant_calling/jobs/
    ...
    echo "" >> "${JOB_FILE}"
    echo "# rm ${PATH_GENOOM}${CHROM}.bed" >> "${JOB_FILE}"
    echo "# rm ${PATH_GENOOM}${CHROM}.dict" >> "${JOB_FILE}"
    echo "# rm ${GENOOM}" >> "${JOB_FILE}"
    echo "# rm ${GENOOM}.fai" >> "${JOB_FILE}"
    echo "# rm ${GENOOM}.amb" >> "${JOB_FILE}"
