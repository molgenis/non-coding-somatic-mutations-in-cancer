...
def close_germline(cursor, position_gene, start, end, chr, where, count_number, set_snps, count_unique):
    # START
    cursor.execute("""
                    SELECT *
                    FROM 'snp'
...
def place_germline(position_gene, cursor, where, value):
    print(value)
    cursor.execute("""
                    SELECT *
                    FROM 'snp'
    ...
    set_snps = set()
    for res in results:
        #close_germline(cursor, position_gene, start, end, where, count_number, set_snps, count_unique)
        count_number, set_snps, count_unique = close_germline(cursor, position_gene, int(res['pos_start']), int(res['pos_end']), int(res['chr']), where, count_number, set_snps, count_unique)
