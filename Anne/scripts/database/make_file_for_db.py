...
def make_plot_format_other(path, basename, out_path):
    ...
    select_columns = ['icgc_donor_id', 'project_code', 'icgc_specimen_id', 'icgc_sample_id', 'chromosome',
                      'chromosome_start',
    ...
    # Rename columns
    select_df.rename(
        columns={'icgc_donor_id': 'donor_id', 'project_code': 'project_id', 'icgc_specimen_id': 'specimen_id',
                 ...
                 'sequencing_strategy': 'seq_strategy', 'mutant_allele_read_count': 'mutant_allele_read_count'},
        inplace=True)
...
def main():
    ...
    # example file name: download?fn=%2Fcurrent%2FProjects%2FALL-US%2Fsimple_somatic_mutation.open.ALL-US.tsv.gz
