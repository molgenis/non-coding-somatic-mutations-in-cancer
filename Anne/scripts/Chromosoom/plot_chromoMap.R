...
chromPlot(gaps=hg_gap, bands=UCSC_gene, colBand = "##9e9f93", annot1=sort_snps,  colAnnot1="brown", figCols=3, scale.title="Counts", title = "UCSC_gene", legChrom=22, chr=c(20:22))

chromPlot(gaps=hg_gap, bands=NCBI_gene, colBand = "##9e9f93", annot1=sort_snps,  colAnnot1="brown", figCols=3, scale.title="Counts", title = "NCBI_gene", legChrom=22, chr=c(20:22))

chromPlot(gaps=hg_gap, bands=V38_gene, colBand = "##9e9f93", annot1=sort_snps,  colAnnot1="brown", figCols=3, scale.title="Counts", title = "V38_gene", legChrom=22, chr=c(20:22))
...
#kp <- plotKaryotype()
#kp <- plotKaryotype(genome = "hg19", chromosomes=c("chr10", "chr12", "chr22")) #chromosomes="autosomal"
# custom genomes
...
snps22<-read.table("SS6004094_merge_mutect2_bowtie.tsv",sep="\t",header=F)
colnames(snps22)<-c("gene_id", "seqnames","Start","End")
snps22$range <- paste(snps22$Start, "-", snps22$End)
genes <- GRanges(snps22)
kp <- plotKaryotype()
kp <- plotKaryotype(genome="hg19", plot.type=2, chromosomes="chr22", main="Gene Density")
kpAddBaseNumbers(kp, tick.dist = 10000000, minor.tick.dist = 1000000, minor.tick.len = 5, minor.tick.col = "gray")
kp <- kpPlotDensity(kp, data=genes, r0=0, r1=0.27, window.size = 100000)
...
kp <- kpPlotDensity(kp, data=genes, r0=0.34, r1=0.61, window.size = 100000)
...
#kp <- kpPlotDensity(kp, genes, window.size = 0.5e6, col="#ddaacc", border="#3388FF")
#sort_snps <- delete_chr(snps22)
