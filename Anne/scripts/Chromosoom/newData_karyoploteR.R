...
create_data_karyoploteR <- function(path, chr) {
...
for (chr in chrom){
  ...
  for (i in 1:length(filenames)){
    if ((i%%5) == 0 || i == 1) {
      ...
      counter <- 0
      name <- paste('D:/Hanze_Groningen/STAGE/bed/PLOTS/kary/', chr, '/plot', i, '.png', sep="")
      print(name)
      ...
      sample1 <- create_data_karyoploteR(filenames[i], chr)
