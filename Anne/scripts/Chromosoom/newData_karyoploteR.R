...
read_files <- function(type_files, filenames, chr, path_save, r0, r1) {
    ...
    for (i in 1:length(filenames)){        
        ...
        if (type_files == 'other'){
            basename_file <- strsplit(basename(filenames[i]), "_", fixed = TRUE)[[1]][1]
        } else {
        ...
        if ( (i%%12) == 0 || i == 1){
            ...
            counter <- 0            
            name <- paste(path_save, type_files, '/', chr, '/plot', i, basename_file, '.png', sep="")
            ...
            if (i == length(filenames)) {
                dev.off()
                name <- paste(path_save, type_files, '/', chr, '/ALLplot', i, basename_file, '.png', sep="")
                peaks_file <- paste(path_save, type_files, '/', chr, '/ALLplot_',basename_file, '_', chr, '.tsv', sep="")
                png(name, width = 2000, height = 1000)
...
main <- function() {
    ...
    for (chr in chrom){
        read_files(type_files, filenames, chr, path_save, r0, r1)        
    }
}
