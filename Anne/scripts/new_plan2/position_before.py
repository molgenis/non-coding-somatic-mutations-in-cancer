...
def close_to(db, gene, chr, start_pos, end_pos, gene_file, donor_dict, donor_list, gene_name_list, sparse_matrix,
             donor_cancer_list):
    """
    ...
    :return: sparse_matrix:   A matrix which contains very few non-zero elements. It contains the counts of a specific
                              region-donor combination.
    ...
    for donor in donor_list_snp:
        ...
        sparse_matrix[donor_index, gene_index] += 1
    ...
    return sparse_matrix, gene_name_list


def write_sparse_matrix(sparse_matrix, gene_name_list, donor_list, save_path, pos, donor_cancer_list):
    """
...
def loop_over_genes(db, gene_df, position_out_gene, position_in_gene, donor_dict, donor_list, donor_cancer_list,
                    save_path, gene_name_bef, gene_name_aft, sparse_matrix_before, sparse_matrix_after):
    """
    ...
    :param sparse_matrix_before:A matrix which contains very few non-zero elements. It contains the counts of a
                                specific region (before gene)-donor combination.
    :param sparse_matrix_after: A matrix which contains very few non-zero elements. It contains the counts of a
    ...
    for index, row in gene_df.iterrows():
        ...
        # Call close_to
        print('BEF')
        sparse_matrix_before, gene_name_bef = close_to(db, row['#name'], chr, row['txStart'] - position_out_gene,
                                                       row['txStart'] + position_in_gene, before_gene_file, donor_dict,
                                                       donor_list, gene_name_bef, sparse_matrix_before,
                                                       donor_cancer_list)
        print('AFT')
        sparse_matrix_after, gene_name_aft = close_to(db, row['#name'], chr, row['txEnd'] - position_in_gene,
                                                      row['txEnd'] + position_out_gene, after_gene_file, donor_dict,
                                                      donor_list, gene_name_aft, sparse_matrix_after, donor_cancer_list)

    # Close file
    before_gene_file.close()
    after_gene_file.close()
    # Call write_sparse_matrix
    write_sparse_matrix(sparse_matrix_before, gene_name_bef, donor_list, save_path, 'bef', donor_cancer_list)
    write_sparse_matrix(sparse_matrix_after, gene_name_aft, donor_list, save_path, 'aft', donor_cancer_list)
...
def main():
    ...
    * Regions of  -2000bp - 250bp (5' UTRs if applicable) from the transcription starting sites (TSS) 
    ...
    sparse_matrix_before = csr_matrix((len(donor_list), len(gene_name_list)),
                                      dtype=np.int8).toarray()
    sparse_matrix_after = csr_matrix((len(donor_list), len(gene_name_list)),
                                    dtype=np.int8).toarray()
    ...
    loop_over_genes(db, gene_df, position_out_gene, position_in_gene, donor_dict, donor_list, donor_cancer_list,
                    save_path, gene_name_list, gene_name_list.copy(), sparse_matrix_before, sparse_matrix_after)
    print('CLOSE')
