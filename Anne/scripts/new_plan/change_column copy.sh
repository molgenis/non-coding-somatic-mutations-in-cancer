...
for filename in ${PATH_DIR}0f*.vcf.gz; do
    echo '-------------------------------------'
    echo "$filename"
    # Get basename
    ...
    # Add sentence
    echo "#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO	FORMAT	NORMAL_${codeName}	TUMOR_${codeName}" >> ${PATH_DIR}header.vcf
    #no headers
    bcftools view --no-header $filename > ${PATH_DIR}nohead.vcf
