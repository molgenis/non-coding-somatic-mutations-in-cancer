...
def close_to(db, gene, chr, start_pos, end_pos, gene_file, donor_dict, donor_list, donor_cancer_list, filter_num):
    ...
    db.cursor.execute("""
                    ...
                    WHERE snp.chr = '%s' AND snp.pos_start >= %s AND snp.pos_end <= %s AND sum_dosage_GT.total_read_count_sum > %s 
                            AND sum_dosage_GT.mutant_allele_read_count_sum > 0 AND snp.ID = sum_dosage_GT.snp_ID AND snp.DNase = 1
...
def main():
    ...
    #
    path_db = config['database_DNase'] #'D:/Hanze_Groningen/STAGE/DATAB/copydatabase_C.db'
    # Database connection
    db = Database(path_db)
    # Call get_projects
