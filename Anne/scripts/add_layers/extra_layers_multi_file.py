...
def layer_run(df_variant, name_variant, df, path_save, chr):    
    ...
    :param df_varaint: The file with the regios of the layer
    ...
    for index, row in df_variant.iterrows():
        ...
        filter_region = df[(df['chr']==row['#Chromosome']) & (df['pos_start']>=row['Start']) & (df['pos_end']<=row['End'])]
        # Get list of donores
        ...
        if len(filter_region) > 0:
            ...
            len(filter_region['snp_ID'])) + '\t' + ','.join(map(str, list(filter_region['snp_ID']))) + '\t' + str(len(donor_list)) + '\t' + str(
